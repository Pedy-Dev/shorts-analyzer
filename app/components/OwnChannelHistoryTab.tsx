// app/components/OwnChannelHistoryTab.tsx
// 내 채널 분석 기록 전용 탭
'use client';
...
export default function OwnChannelHistoryTab({ isLoggedIn }: OwnChannelHistoryTabProps) {
  ...
  const loadHistory = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/analysis-history/own/list');
      const data = await response.json();

      if (data.success) {
        setHistoryList(data.history || []);
        console.log(`✅ [내 채널] ${data.count}개 분석 기록 로드 완료`);
      } else if (response.status === 401) {
        console.log('ℹ️ 로그인이 필요합니다');
      } else {
        console.error('❌ 내 채널 분석 기록 조회 실패:', data.error);
      }
    } catch (error) {
      console.error('❌ 내 채널 분석 기록 조회 오류:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      {loading ? (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-gray-600">내 채널 분석 기록을 불러오는 중...</span>
        </div>
      ) : historyList.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <Film className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-600">아직 분석한 내 채널이 없습니다</p>
          <p className="text-gray-500 text-sm mt-2">내 채널을 분석하면 여기에 기록이 표시됩니다</p>
        </div>
      ) : (
        <div className="space-y-4">
                ...
                {isExpanded && (
                  <div className="border-t p-6 bg-gray-50">
                    {isLoading ? (
                      ...
                      <OwnChannelAnalysisDetails record={recordDetail} />
                    ) : (
